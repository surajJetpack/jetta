...
async function main() {
  const r = await sendEscalation({
    ...
    headline: "Signed-status callback never fires for one account",
    app: "getsign",
    accountLabel: "dana@acme-corp.com",
    ticketRef: "#13598",
    summary:
      "GetSign signed document completes but the monday item status stays 'Pending Signature' instead of updating (request #GS-4471).",
    alreadyTried:
      "Searched KB — no matching article\nConfirmed the account and the signature request completed\nAsked the user for the board URL and Status/Files column names",
    question:
